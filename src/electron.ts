 ...
 *
 * Re-exports every Sambar module under Electron's names and wraps them in a
 ...
 * (matching a plain object). Consumers alias `electron` → `sambar` so
...
/**
 * Wrap `base` (the Sambar module surface) so unimplemented Electron module names
 * throw an actionable error on access. Injectable for testing.
 */
export const createElectronShim = (
  base: Record<string, unknown> = sambar as unknown as Record<string, unknown>,
